...
# Prod lockdown override. Without this ARG the variable never reaches `next build`,
# so middleware.ts falls back to host matching and updown.my stays on /soon no
# matter what Railway's Variables tab says.
ARG NEXT_PUBLIC_PROD_LOCKDOWN
# web → terminal link
...
ENV NEXT_PUBLIC_PROD_LOCKDOWN=$NEXT_PUBLIC_PROD_LOCKDOWN
