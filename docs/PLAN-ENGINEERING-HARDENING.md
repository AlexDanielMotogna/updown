...
- **P0.2 — ✅ NÚCLEO COMPLETO (4 slices).** Suite de tests sobre la lógica de dinero. Era ~1 test. El cálculo parimutuel, `resolve`, `refund`, `refund_bettor`, `claim`, `close_*` no tienen tests. Un bug aquí mueve fondos. → tests unitarios del cálculo + tests de integración del scheduler con un validador local / mocks de Connection.
  ...
  - **Slice 5 — ✅ (commit `8e29c8f`).** `apps/api/src/utils/levels-fees.test.ts` (13 tests): `getFeeBps` (escalones L<5..L40, monótono, default), curva XP de `levels.ts` (round-trip `getLevelForXp`∘`getXpForLevel`, monotonía, cap, gaps), títulos/multiplicador por banda, `winStreakBonus`. Pura, money-adjacent (level→fee→payout). **74 tests nuevos en total.**
  - **P0.2 — núcleo COMPLETO.** Matemática on-chain (claim/refund/multiplier/conservación) + reglas de ganador off-chain (sports/crypto) + safety de void + fee/level math. Lo que queda fuera de P0.2-núcleo: arreglar el harness vitest (env/DB) para correr la suite entera; tests de integración con validador local (litesvm/bankrun) para los handlers completos de las instrucciones.
