...
export function UserLevelBadge({ level, title, size = 'sm', variant = 'default' }: UserLevelBadgeProps) {
  ...
  if (variant === 'icon-only') {
    return (
      <Box component="img" src={icon} alt={`Level ${level}`} sx={{ width: isSm ? 24 : 32, height: isSm ? 24 : 32 }} />
    );
  }

  return (
    <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.5, px: isSm ? 0.5 : 1, py: isSm ? 0.25 : 0.5 }}>
      <Box component="img" src={icon} alt={`Level ${level}`} sx={{ width: isSm ? 28 : 36, height: isSm ? 28 : 36 }} />
      <Typography sx={{ fontSize: isSm ? '0.65rem' : '0.75rem', fontWeight: 700, color, lineHeight: 1 }}>Lv.{level}</Typography>
      {!isSm && (
        <Typography sx={{ fontSize: '0.65rem', fontWeight: 500, color: withAlpha(color, 0.8), lineHeight: 1 }}>{title}</Typography>
      )}
    </Box>
  );
}
