...
function BalanceInner() {
  ...
  return (
    <span className="flex h-[38px] items-center gap-1 rounded-md bg-white/[0.06] px-3 text-[0.7rem] font-semibold tabular text-surface-100 sm:text-[0.8rem]" title="HyperLiquid account value (spot + perps)">
      <span className="text-win-500">$</span>
      {ready ? total.toLocaleString(undefined, { maximumFractionDigits: 2 }) : '0.00'}
    </span>
  );
}
