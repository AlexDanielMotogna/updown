...
export default function WorldCupPage() {
  ...
  return (
    <Box sx={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', bgcolor: t.bg.app, color: t.text.primary, overflowX: 'hidden' }}>
      ...
      {/* Footer: how-to-win rule + disclaimer */}
      <Box sx={{ borderTop: `1px solid ${t.border.subtle}`, mt: 2 }}>
        <Box sx={{ maxWidth: 1400, mx: 'auto', px: { xs: 2, md: 3 }, py: 2.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 1.75, p: 1.5, borderRadius: 1.5, bgcolor: `${WC_NEON_GREEN}12`, border: `1px solid ${WC_NEON_GREEN}40` }}>
            <InfoOutlined sx={{ fontSize: 18, color: WC_NEON_GREEN, flexShrink: 0, mt: '1px' }} />
            <Typography sx={{ fontSize: { xs: '0.8rem', md: '0.85rem' }, color: t.text.primary, lineHeight: 1.55 }}>
              <Box component="span" sx={{ fontWeight: 800 }}>How to win:</Box> predict the <Box component="span" sx={{ fontWeight: 800, color: WC_NEON_GREEN }}>exact final score</Box> and phase (regulation 90&apos;, extra time, or penalties). Close guesses don&apos;t count — only an exact match qualifies. Example: if you pick <Box component="span" sx={{ fontWeight: 700 }}>2-1</Box> and it ends <Box component="span" sx={{ fontWeight: 700 }}>3-1</Box>, you don&apos;t win.
            </Typography>
          </Box>
