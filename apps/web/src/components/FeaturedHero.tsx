...
import { ChevronLeft, ChevronRight, ShowChart, SportsSoccer } from '@mui/icons-material';
...
export function FeaturedHero({ pools, categoryMap, onSelect }: Props) {
  ...
  useEffect(() => {
    if (count <= 1) return;
    const iv = setInterval(() => setIndex(i => (i + 1) % count), 8000);
  ...
  const catIcon: ReactNode = isCrypto ? <ShowChart sx={{ fontSize: 16 }} /> : CatIcon ? <CatIcon sx={{ fontSize: 16 }} /> : !isCrypto && !isPrediction ? <SportsSoccer sx={{ fontSize: 16 }} /> : null;
  ...
  const outcomes: Array<{ name: string; color: string; pct: number }> = isCrypto
    ? [{ name: 'Up', color: t.up, pct: pct(totalUp, 50) }, { name: 'Down', color: t.down, pct: pct(totalDown, 50) }]
    : isPrediction
      ...
      : [
          { name: pool.homeTeam || 'Home', color: t.up, pct: pct(totalUp, isTwoWay ? 50 : 33) },
          ...(!isTwoWay ? [{ name: 'Draw', color: t.draw, pct: pct(totalDraw, 34) }] : []),
          { name: pool.awayTeam || 'Away', color: t.down, pct: pct(totalDown, isTwoWay ? 50 : 33) },
        ];
  ...
  return (
    <Box sx={{ bgcolor: t.bg.surface, border: t.surfaceBorder, borderRadius: 2, p: { xs: 1.75, md: 2.5 }, mb: { xs: 3, md: 4 } }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '300px 1fr' }, gap: { xs: 2, md: 3 }, alignItems: 'start' }}>
        ...
        <Box sx={{ minWidth: 0 }}>
          <OddsChart key={pool.id} poolId={pool.id} totalUp={pool.totalUp} totalDown={pool.totalDown} />
        </Box>
      </Box>
    </Box>
  );
